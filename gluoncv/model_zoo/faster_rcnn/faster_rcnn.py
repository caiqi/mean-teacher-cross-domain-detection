...
class FasterRCNN(RCNN):
    ...
    def __init__(self, features, top_features, classes,
        ...
        if "use_vgg" in kwargs:
            self.use_vgg = kwargs["use_vgg"]
            del kwargs["use_vgg"]
        else:
            self.use_vgg = 0

        if "use_sampled_for_adver" in kwargs:
            self.use_sampled_for_adver = kwargs["use_sampled_for_adver"]
            del kwargs["use_sampled_for_adver"]
        else:
            self.use_sampled_for_adver = 0

        if "neg_iou_thres" in kwargs:
            self.neg_iou_thres = kwargs['neg_iou_thres']
            del kwargs['neg_iou_thres']
        else:
            self.neg_iou_thres = -1.

        if "output_feature" in kwargs:
            self._output_feature = kwargs['output_feature']
            del kwargs['output_feature']
        else:
            self._output_feature = 0
        ...
        with self.name_scope():
            ...
            self.sampler = RCNNTargetSampler(
                ...
                neg_iou_thres=self.neg_iou_thres)
    ...
    def hybrid_forward(self, F, x, gt_box=None, precomputed_rois=None):
        ...
        if autograd.is_training():
            ...
            if gt_box is None:
                rpn_box = rpn_box[:, 0:self._num_sample, :]
                samples = rpn_score[:, 0:self._num_sample, :]
                matches = F.zeros_like(F.mean(rpn_box, axis=-1))
            else:
                rpn_box, samples, matches = self.sampler(rpn_box, rpn_score, gt_box)
            if precomputed_rois is not None:
                rpn_box = precomputed_rois
        else:
        ...
        if self.use_vgg:
            ...
            avg_feat = self.top_features(pooled_feat)
        else:
        ...
        if self._additional_output and autograd.is_training():
            raise NotImplementedError
        ...
        if autograd.is_training():
            ...
            if self._output_feature:
                return (cls_pred, box_pred, rpn_box, samples, matches,
                        raw_rpn_score, raw_rpn_box, anchors, feat, avg_feat)
            return (cls_pred, box_pred, rpn_box, samples, matches,
...
def faster_rcnn_resnet50_v1b_voc(pretrained=False, pretrained_base=True, **kwargs):
    ...
    return get_faster_rcnn(
        ...
        features=features, top_features=top_features, classes=classes,
        short=300, max_size=500, train_patterns=train_patterns,
        nms_thresh=0.3, nms_topk=400, post_nms=100,
        roi_mode='align', roi_size=(14, 14), stride=16, clip=None,
...
def faster_rcnn_resnet50_v1b_custom(classes, transfer=None, pretrained_base=True,
    ...
    if transfer is None:
        ...
        base_network = resnet50_v1b(pretrained=pretrained_base, dilated=False,
                                    use_global_stats=True, root=kwargs['root'])
        features = nn.HybridSequential()
        top_features = nn.HybridSequential()
        for layer in ['conv1', 'bn1', 'relu', 'maxpool', 'layer1', 'layer2', 'layer3']:
            features.add(getattr(base_network, layer))
...
def faster_rcnn_vgg16_custom(classes, transfer=None, pretrained_base=True,
                             pretrained=False, **kwargs):
    r"""Faster RCNN model with resnet50_v1b base network on custom dataset.
    ...
    if transfer is None:
        ...
        base_network = vgg16(pretrained=pretrained_base, root=kwargs['root'])
        features = nn.HybridSequential()
        top_features = nn.HybridSequential()
        features.add(getattr(base_network, "features")[0:30])
        top_features.add(getattr(base_network, "features")[31:35])
        train_patterns = '|'.join(['.*dense', '.*rpn',
                                   '.*conv(7|8|9|10|11|12)_conv'])
        return get_faster_rcnn(
            name='vgg16', dataset='custom', pretrained=pretrained,
            features=features, top_features=top_features, classes=classes,
            short=600, max_size=1000, train_patterns=train_patterns,
            nms_thresh=0.3, nms_topk=400, post_nms=100,
