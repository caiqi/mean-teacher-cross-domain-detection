...
class RCNNTargetSampler(gluon.HybridBlock):
    ...
    def __init__(self, num_image, num_proposal, num_sample, pos_iou_thresh, pos_ratio, neg_iou_thres=-1.):
        ...
        self.neg_iou_thres = neg_iou_thres
    ...
    def hybrid_forward(self, F, rois, scores, gt_boxes):
        ...
        with autograd.pause():
            ...
            for i in range(self._num_image):
                ...
                # when iou is less or equal than self.neg_iou_thres, mark it as ignore samples
                mask = F.where(ious_max <= self.neg_iou_thres, F.zeros_like(mask), mask)
                ...
                # shuffle mask
                rand = F.random.uniform(0, 1, shape=(self._num_proposal + 2000,))
